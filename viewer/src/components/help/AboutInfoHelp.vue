...
const aboutInfoContent = [
	...
	{
		title: 'About ERA5',
		subtitle: 'Spatially complete reanalysis from 1950',
		icon: 'IconWorldHeart',
		content: `<p> All of the climate statistics generated and visualised by this application are derived from the&nbsp;<b>ERA5</b>&nbsp;reanalysis dataset, which is available to download and use under the CC-BY 4.0 licence&nbsp;<a href="https://cds.climate.copernicus.eu/datasets/derived-era5-single-levels-daily-statistics?tab=overview" target="_blank" >here</a >.&nbsp;<b>ERA5</b>&nbsp;is a global&nbsp;<b>atmospheric reanalysis</b >&nbsp;that covers the period from 1940 to present and is developed and maintained by&nbsp;<b>ECMWF</b>. It combines direct observations with a numerical model to create a global estimate of various climate variables. This application uses the post-processed&nbsp;<b>daily statistics, </b>on a regular 0.25° × 0.25° grid. Map layers show data&nbsp;from 1979-present. </p> <p> The information presented by this application uses a clustering algorithm to group individual grid-cells associated with extreme events (e.g. heatwaves and coldwaves) for specific locations. Individual pixels are from the post-processed&nbsp;<b>ERA5</b>&nbsp;reanalysis and is not based on site-specific observations. Individual grid cells on the interpolated grid are typically several hundred square kilometres in size and represent the average environment at that location. Extreme event summary statistics are calculated from all pixels associated with a particular event. </p> <p> There are several notable caveats to consider when interpreting the&nbsp;<b>ERA5</b>&nbsp;data presented here: Due to changes in the observational network through time, limited observations in the early decades decrease the reliability of&nbsp;<b>ERA5</b>&nbsp;during that period (<a href="https://doi.org/10.1002/qj.4803" target="_blank" >Soci et al., 2024</a >). As such, users wishing to draw conclusions about long-term climate trends downloaded from the app should be aware of the caveats and limitations related to&nbsp;<b>ERA5</b>&nbsp;data and are invited to consult the documentation. The explorer is designed to make exploring&nbsp;<b>extreme events</b>&nbsp;a faster and easier task, but should only be a first step in any scientific analysis. </p> <p> For further information, the user is encouraged to begin with the&nbsp;<a href="https://climate.copernicus.eu/reanalysis-qas" target="_blank" >Reanalysis FAQs</a >. </p> `,
	},
	{
		title: 'About this data',
		subtitle: 'How to interpret this data',
		icon: 'IconChartHistogram',
		content: `<p>Temperature refers to the&nbsp;<b>2 metre temperature</b>&nbsp;in&nbsp;<b>ERA5</b>, which represents the air temperature at 2 metres above the surface. Over land, near-surface temperature observations from weather stations are combined with the model output to improve accuracy. The 2 metre temperature is influenced by environmental factors like land type and vegetation. Note that&nbsp;<b>ERA5</b>&nbsp;does not explicitly represent urban environments, meaning it may not fully capture the&nbsp;<i>urban heat island</i>&nbsp;effect commonly observed in cities. </p> <p>The daily aggregation statistics used are daily maximum temperature for heatwaves, and daily minimum temperature for coldwaves. For more information see the <a href="https://confluence.ecmwf.int/display/CKB/ERA5+family+post-processed+daily+statistics+documentation" target="_blank">documentation</a></p> <p>
		Events are detected using a clustering algorithm, which groups together spatiotemporally nearby grid cells which:<p>Exceed the 99th/1st percentile value for that location (average daily maximum at that cell over the period 1991-2020).</p><p>Exceed an absolute threshold of 28°C/0°C</p><p>Do so for at least 3 consecutive days</p><p>See <a href="${import.meta.env.X_PUBLIC_PATH}Clustering%20Algorithm%20Description.pdf" target="_blank">the clustering algorithm description</a> for further details.</p>`,
	},
	{
		title: 'Disclaimer',
			...
			<div class="drawer-front">
				<div class="label">
					...
					<div class="label-text">
						...
						<h2>Unlock the power of ERA5!</h2>
					</div>
				</div>
				<button
					class="expand-small glassy flat"
					:aria-label="
						store.aboutSectionOpen === 'about'
							? 'Collapse section'
							: 'Expand section'
					"
					@click="
						store.aboutSectionOpen =
							'about' === store.aboutSectionOpen ? null : 'about'
					"
				>
					<IconChevronDown
						class="icon"
						:class="{ open: store.aboutSectionOpen === 'about' }"
						aria-hidden="true"
					/>
				</button>
			</div>
			...
			>
				<div class="mainmodes">
					<div class="timemachine">
						...
						<h2>Time Machine</h2>
						<p>
							Explore historical extreme temperature events by selecting a year
							from the timeline. Click on the map to view detailed statistics
							for specific locations affected by these events.
						</p>
					</div>
					<div class="heatmap">
						<div class="button glassy color decoration">
						...
						<h2>Overview</h2>
						<p>
							Visualize the frequency and intensity of extreme temperature
							events over the entire dataset period. Use the color scale to
							identify hotspots and trends in extreme temperatures.
						</p>
					</div>
				</div>
			</div>
