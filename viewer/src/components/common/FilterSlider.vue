...
watch(model, (v) => {
  if (isBand.value) {
    if (Array.isArray(v)) {
      ...
      if (lowVal.value > highVal.value) [lowVal.value, highVal.value] = [highVal.value, lowVal.value]
    } else {
...
const lowPct = computed(() => ((lowVal.value - props.min) / (props.max - props.min)) * 100)
const highPct = computed(() => ((highVal.value - props.min) / (props.max - props.min)) * 100)
    ...
    <template v-else>
      <div class="range">
        ...
        <span>
          ...
          <template v-else-if="props.type === 'low-pass'">≤ {{ highVal }}</template>
        </span>
      </div>
    </template>
  </div>
